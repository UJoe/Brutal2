      ...
      `;
				...
				case "teri":
					if (opera === 0) {
						...
						let td = [
							...
							"Akadályozza a mozgást, de a lövést nem.",
						];
						if (featuredU > -1) {
		...
		function prÁll(u) {
			...
			if (ar.length > 0) {
				...
				if (
					u.name !== "Robi" &&
					u.cr < Math.random() * 3 + (u.spec === "részegség") + (u.hp === u.ohp) + (u.name === "Óriás")
				) {
					activate(u);
					if (u.id === featuredU) updateFeatured(u);
				}
			...
			switch (opera) {
				case 0:
					...
					document
						...
						.forEach((s) => s.style.setProperty("transition", `left linear ${1 / gspeed}s, top linear ${1 / gspeed}s`));
					break;
